package com.yunhu.yhshxc.bo;

import java.util.List;



/**
 * 考勤Bean
 * 
 * @version 2013.6.3
 * @author wangchao
 *
 */
public class AttendanceNew {

	/**
	 * 用户名
	 */
	private String userName;

	/**
	 * 上班时间
	 */
	private String inTime;

	/**
	 * 上班考勤的定位地址
	 */
	private String inAddress;

	/**
	 * 上班考勤的纬度
	 */
	private String inLat;

	/**
	 * 上班考勤的经度
	 */
	private String inLon;

	/**
	 * 上班考勤的定位类型
	 */
	private String inGisType;

	/**
	 * 上班考勤的照片
	 */
	private String inImage;

	/**
	 * 上班考勤说明
	 */
	private String inComment;

	/**
	 * 下班考勤时间
	 */
	private String outTime;

	/**
	 * 下班考勤的定位地址
	 */
	private String outAddress;

	/**
	 * 下班考勤的纬度
	 */
	private String outLat;

	/**
	 * 下班考勤的经度
	 */
	private String outLon;

	/**
	 * 下班考勤的定位类型
	 */
	private String outGisType;

	/**
	 * 下班考勤的照片
	 */
	private String outImage;

	/**
	 * 下班考勤的说明
	 */
	private String outComment;

	/**
	 * 考勤日期
	 */
	private String attendTime;
	
	/**
	 * 考勤类型
	 */
	private String attendType;
	
	/**
	 * 报岗数据
	 * @return
	 */
	private List<AttendanceGang> gangList;
	

	public List<AttendanceGang> getGangList() {
		return gangList;
	}

	public void setGangList(List<AttendanceGang> gangList) {
		this.gangList = gangList;
	}

	public String getUserName() {
		return userName;
	}

	public void setUserName(String userName) {
		this.userName = userName;
	}

	public String getInTime() {
		return inTime;
	}

	public void setInTime(String inTime) {
		this.inTime = inTime;
	}

	public String getInAddress() {
		return inAddress;
	}

	public void setInAddress(String inAddress) {
		this.inAddress = inAddress;
	}

	public String getInLat() {
		return inLat;
	}

	public void setInLat(String inLat) {
		this.inLat = inLat;
	}

	public String getInLon() {
		return inLon;
	}

	public void setInLon(String inLon) {
		this.inLon = inLon;
	}

	public String getInGisType() {
		return inGisType;
	}

	public void setInGisType(String inGisType) {
		this.inGisType = inGisType;
	}

	public String getInImage() {
		return inImage;
	}

	public void setInImage(String inImage) {
		this.inImage = inImage;
	}

	public String getInComment() {
		return inComment;
	}

	public void setInComment(String inComment) {
		this.inComment = inComment;
	}

	public String getOutTime() {
		return outTime;
	}

	public void setOutTime(String outTime) {
		this.outTime = outTime;
	}

	public String getOutAddress() {
		return outAddress;
	}

	public void setOutAddress(String outAddress) {
		this.outAddress = outAddress;
	}

	public String getOutLat() {
		return outLat;
	}

	public void setOutLat(String outLat) {
		this.outLat = outLat;
	}

	public String getOutLon() {
		return outLon;
	}

	public void setOutLon(String outLon) {
		this.outLon = outLon;
	}

	public String getOutGisType() {
		return outGisType;
	}

	public void setOutGisType(String outGisType) {
		this.outGisType = outGisType;
	}

	public String getOutImage() {
		return outImage;
	}

	public void setOutImage(String outImage) {
		this.outImage = outImage;
	}

	public String getOutComment() {
		return outComment;
	}

	public void setOutComment(String outComment) {
		this.outComment = outComment;
	}

	public String getAttendTime() {
		return attendTime;
	}

	public void setAttendTime(String attendTime) {
		this.attendTime = attendTime;
	}

	public String getAttendType() {
		return attendType;
	}

	public void setAttendType(String attendType) {
		this.attendType = attendType;
	}
	
}
