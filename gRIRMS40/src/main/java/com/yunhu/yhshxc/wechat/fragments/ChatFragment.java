 ...
 */
@SuppressLint("NewApi")
public class ChatFragment extends Fragment implements ExpendListViewOnlick{
...
//	private List<Topic> historyTopics = new ArrayList<Topic>();//历史话题
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View v = inflater.inflate(R.layout.wechat_chat, null);
	...
	private void initRefreshData(){
		childrenData.clear();
			pubTopics = topicDb.findTopicListForPublic(DateUtil.getCurDate(),0);
		
			bumenTopics = topicDb.findTopicListForBuM(DateUtil.getCurDate(), 0);
...
//				Toast.makeText(getActivity(), "groupPosition "+groupPosition+"  childPosition "+childPosition, 0).show();
					...
					if(topic!=null){
						int isHistory = 0;
						if(topic.getType().equals(Topic.TYPE_1)){//聊天
							Intent intent = new Intent(getActivity(),ExchangeActivity.class);
							intent.putExtra("topicId", topic.getTopicId());
							intent.putExtra("isHistory", isHistory);
							startActivity(intent);
						}else if(topic.getType().equals(Topic.TYPE_2)){//调查
							Intent intent = new Intent(getActivity(),SurveyActivity.class);
							intent.putExtra("topicId", topic.getTopicId());
							intent.putExtra("isHistory", isHistory);
							startActivity(intent);
						} else if(topic.getType().equals(Topic.TYPE_3)){//审批
...
//			showPopup(v);
	...
	class MyExpendableAdapter extends BaseExpandableListAdapter{
		...
		// 设置组视图的显示文字
					private String[] groupTypes = new String[] { PublicUtils.getResourceString(context,R.string.wechat_content22), PublicUtils.getResourceString(context,R.string.wechat_content23)};
					private int[] groupImages = new int[]{R.drawable.wechat_public,R.drawable.wechat_bumen};
					// 子视图显示文字
...
//						}
						slideView.shrink();
//						slideView.getBack().setText("删除");
						slideView.setPostion(groupPosition,childPosition);
//						if(childPosition == childrenData.get(groupPosition).size()){
//							try {
//								view.initData(null,replyDb,true);
...
//			submitDelete(topic);
	...
	public void refresh(List<List<Topic>> childrenDat,List<Topic> pubTopics,List<Topic> bumenTopics){
