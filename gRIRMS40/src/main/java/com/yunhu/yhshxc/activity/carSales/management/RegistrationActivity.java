...
public class RegistrationActivity extends BaseActivity implements OnList{
	...
	private RegistrationTreeAdapter mAdapter;
	private RegistrationTreeAdapter commonAdapter;
	 ...
	 /**
		...
		private void initCommonProductCtrl() throws IllegalArgumentException, IllegalAccessException {
			...
			if (productCtrlList.size() > 0) {
				...
				commonAdapter = new RegistrationTreeAdapter<CarSalesProductCtrl>(lv_xiadan, RegistrationActivity.this, productCtrlList, 10,RegistrationActivity.this);
					...
					public void onClick(Node node, int position) {
						if(node.isLeaf()){
							if(!node.getProductCtrl().isProductLevel()){
								ToastOrder.makeText(RegistrationActivity.this, "没有可选购产品", ToastOrder.LENGTH_LONG).show();
							}
						}
					}
				});
		...
		private void initAllProductCtrl() throws IllegalArgumentException, IllegalAccessException{
			...
			if (productCtrlList!=null&&productCtrlList.size() > 0) {
				mAdapter = new RegistrationTreeAdapter<CarSalesProductCtrl>(lv_all, RegistrationActivity.this, productCtrlList, 10,RegistrationActivity.this);
		...
		private void goStockTakingList() {
			...
			if(productCtrlsChild!=null&&productCtrlsChild.size()>0){
				Intent intent = new Intent(RegistrationActivity.this,RegistrationListActivity.class);
				startActivityForResult(intent, 112);
			}else{
				ToastOrder.makeText(RegistrationActivity.this, "您没选择产品", ToastOrder.LENGTH_LONG).show();
			}
...
//							im.toggleSoftInput(0, InputMethodManager.SHOW_FORCED);
							...
							ToastOrder.makeText(RegistrationActivity.this, "添加成功", ToastOrder.LENGTH_LONG).show();
						}
						return;
					}else{
						ToastOrder.makeText(RegistrationActivity.this, "缺货列表已存在", ToastOrder.LENGTH_LONG).show();
						return;
					}
//					for(int i = 0; i<ctrls.size(); i++){
//						CarSalesProductCtrl c = ctrls.get(i);
//						if(c.getId() == ctrl.getId()&&!TextUtils.isEmpty(c.getOutCount())&&!c.getOutCount().equalsIgnoreCase("0")&&!c.getOutCount().equalsIgnoreCase("0.0")){
//							Toast.makeText(RegistrationActivity.this, "缺货列表已存在", Toast.LENGTH_LONG).show();
//							return;
//						}else{
//							int key = arr.keyAt(i);
...
//					initProduct(product);
//					Intent i = new Intent(RegistrationActivity.this,RegistrationListActivity.class);
//					startActivity(i);
				} else {
					ToastOrder.makeText(RegistrationActivity.this, "该产品不存在，请确认是否扫描错误",ToastOrder.LENGTH_LONG).show();
				}
			}
		}
		...
		public boolean onKeyDown(int keyCode, KeyEvent event) {
			if (keyCode == KeyEvent.KEYCODE_BACK && event.getAction() == KeyEvent.ACTION_DOWN) {
				...
				Iterator<String> it = set.iterator();
				while(it.hasNext()){
					String s =ctrls.get(it.next()).getOutCount();
					if(!TextUtils.isEmpty(s)&&Double.parseDouble(s)!=0){
						count += 1;
					}
				}
				if (count > 0 ) {
					backDialog("退出后将清除缺货列表数据,是否确定退出?").show();
					return true;
				}else{
					finish();
				}
			...
			public void onReceive(Context context, Intent intent) {
				if (intent!=null && Constants.BROADCAST_CARSALES_CREATE_SUCCESS.equals(intent.getAction())) {
					try {
						...
						ToastOrder.makeText(RegistrationActivity.this, "常用列表数据更新异常", ToastOrder.LENGTH_SHORT).show();
					}
				}
			}
