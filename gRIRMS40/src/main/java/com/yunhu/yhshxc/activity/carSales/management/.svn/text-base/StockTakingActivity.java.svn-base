 ...
 *
 */
public class StockTakingActivity extends BaseActivity implements OnList {
	...
	private void initCommonProductCtrl() throws IllegalArgumentException, IllegalAccessException {
		...
		if (productCtrlList.size() > 0) {
			...
			commonAdapter = new StockTakingTreeAdapter<CarSalesProductCtrl>(lv_xiadan, StockTakingActivity.this,
					productCtrlList, 10, StockTakingActivity.this);
				...
				public void onClick(Node node, int position) {
					if (node.isLeaf()) {
						if (!node.getProductCtrl().isProductLevel()) {
							ToastOrder.makeText(StockTakingActivity.this, "没有可选购产品", ToastOrder.LENGTH_LONG).show();
						}
					}
				}
			});
	...
	private void initAllProductCtrl() throws IllegalArgumentException, IllegalAccessException {
		...
		if (productCtrlList != null && productCtrlList.size() > 0) {
			mAdapter = new StockTakingTreeAdapter<CarSalesProductCtrl>(lv_all, StockTakingActivity.this,
					productCtrlList, 10, StockTakingActivity.this);
	...
	private void goStockTakingList() {
		...
		if (productCtrlsChild != null && productCtrlsChild.size() > 0) {
			...
			ToastOrder.makeText(StockTakingActivity.this, "您没选择产品", ToastOrder.LENGTH_LONG).show();
		}
			...
			if (product != null) {
				...
				if (!ctrls.containsKey(ctrl.getId())) {
					...
					if (et != null) {
						...
						ToastOrder.makeText(StockTakingActivity.this, "添加成功", ToastOrder.LENGTH_LONG).show();
					}
					return;
				} else {
					ToastOrder.makeText(StockTakingActivity.this, "库存列表已存在", ToastOrder.LENGTH_LONG).show();
					return;
				}
				// for(int i = 0; i<ctrls.size(); i++){
				// CarSalesProductCtrl c = ctrls.get(i);
				// if(c.getId() ==
				// ctrl.getId()&&!TextUtils.isEmpty(c.getInverty())){
				// Toast.makeText(StockTakingActivity.this, "库存列表已存在",
				...
				// initProduct(product);
				// Intent i = new
				// Intent(StockTakingActivity.this,StockTakingListActivity.class);
				// startActivity(i);
			} else {
				ToastOrder.makeText(StockTakingActivity.this, "该产品不存在，请确认是否扫描错误", ToastOrder.LENGTH_LONG).show();
			}
		}
	}
		...
		while (it.hasNext()) {
			...
			if (!TextUtils.isEmpty(bundle.getString(key))) {
				if (ctrls.get(key) != null) {
					ctrls.get(key).setInverty(bundle.getString(String.valueOf(key)));
				} else {
					CarSalesProductCtrl c = ctrlDB.findProductCtrlById(Integer.parseInt(key));
					c.setInverty(bundle.getString(String.valueOf(key)));
					if (c != null) {
						ctrls.put(key, c);
					}
				}
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		if (keyCode == KeyEvent.KEYCODE_BACK && event.getAction() == KeyEvent.ACTION_DOWN) {
			...
			Iterator<String> it = set.iterator();
			while (it.hasNext()) {
				String s = ctrls.get(it.next()).getInverty();
		...
		public void onReceive(Context context, Intent intent) {
			if (intent != null && Constants.BROADCAST_CARSALES_CREATE_SUCCESS.equals(intent.getAction())) {
				try {
					...
					ToastOrder.makeText(StockTakingActivity.this, "常用列表数据更新异常", ToastOrder.LENGTH_SHORT).show();
				}
			}
		}
