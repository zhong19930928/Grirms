 ...
 *
 */
public class DoApproLeaveActivity extends AbsBaseActivity implements OnItemClickListener{
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		tv_attend_leave.setText(PublicUtils.getResourceString(this,R.string.leave_m13));
		init();
	...
	private void init() {
		...
		adapter = new PreviewLeaveAdapter(this, list);
						...
						try {
							...
							if ("0000".equals(resultCode)) {
								...
								if ("[]".endsWith(arrayStr)) {
									...
									if (page == 1) {// 说明是刷新
										list.clear();
										newItems = new LeaveUtils(DoApproLeaveActivity.this).paseLeaveInfos(array,true);
									}else{
										newItems = new LeaveUtils(DoApproLeaveActivity.this).paseLeaveInfos(array,false);
									}
...
//		param.put("userId", SharedPreferencesUtil.getInstance(this).getUserId());
	...
	public void onItemClick(AdapterView<?> parent, View view, int position,
		...
		if(list.size()>0){
			...
			Intent intent = new Intent(DoApproLeaveActivity.this,DoDeatailLeaveActivity.class);
//			intent.putExtra("pages", page);
