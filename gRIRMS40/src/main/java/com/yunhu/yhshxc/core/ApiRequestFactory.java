...
public class ApiRequestFactory {
    ...
    */
    private static void init(Context context) {
        if (!NetWorkUtils.isConnect(context)) {
            com.yunhu.yhshxc.core.ToastUtils.disPlayShort(context, "网络连接已断开，请检查网络连接");
            return;
        }
    }
    ...
    */
    ...
    public static void postJson2(final Context context,final String url, Map<String,String> params ,final HttpCallBackListener httpCallBackListener,boolean isShowDialog){
        init(context);
        if (isShowDialog) {
            if (loadingDialog == null) {
                loadingDialog = new SpotsDialog(context,"正在加载数据");
    ...
    */
    public static void postMixture(final Context context, final String url, List<SubmitFile> submitFiles, Map<String,String> params , final HttpCallBackListener httpCallBackListener, boolean isShowDialog){
        ...
        if (isShowDialog) {
            if (loadingDialog == null) {
                loadingDialog = new SpotsDialog(context,"正在上传数据");
            }
            loadingDialog.dismiss();
            loadingDialog.show();
        }
