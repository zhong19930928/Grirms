 ...
 *
 */
public class UnloadingActivity extends BaseActivity implements OnList{
	...
	private UnLoadingTreeAdapter mAdapter;
	private UnLoadingTreeAdapter commonAdapter;
	...
	private void initLoadingGooodsCount() {
		int count = 0;
		 for (Map.Entry<String, CarSalesProductCtrl> entry : ctrls.entrySet()) {
			  ...
			  String s = ctrl.getUnLoadingCount();
	...
	private void initCommonProductCtrl() throws IllegalArgumentException, IllegalAccessException {
		...
		if (productCtrlList.size() > 0) {
			...
			commonAdapter = new UnLoadingTreeAdapter<CarSalesProductCtrl>(lv_xiadan, UnloadingActivity.this, productCtrlList, 10,UnloadingActivity.this,true);
				...
				public void onClick(Node node, int position) {
					if(node.isLeaf()){
						if(!node.getProductCtrl().isProductLevel()){
							ToastOrder.makeText(UnloadingActivity.this, "没有可选购产品", ToastOrder.LENGTH_LONG).show();
						}
					}
				}
			});
	...
	private void initAllProductCtrl() throws IllegalArgumentException, IllegalAccessException{
		...
		if (productCtrlList!=null&&productCtrlList.size() > 0) {
			mAdapter = new UnLoadingTreeAdapter<CarSalesProductCtrl>(lv_all, UnloadingActivity.this, productCtrlList, 10,UnloadingActivity.this,true);
...
//			Intent i = new Intent(this, CaptureActivity.class);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		}else if(requestCode == 111&&resultCode == 222){
...
//						im.toggleSoftInput(0, InputMethodManager.SHOW_FORCED);
						...
						ToastOrder.makeText(UnloadingActivity.this, "添加成功", ToastOrder.LENGTH_LONG).show();
					}
					return;
				}else{
					ToastOrder.makeText(UnloadingActivity.this, "卸车列表已存在", ToastOrder.LENGTH_LONG).show();
					return;
				}
//				for(int i = 0; i<ctrls.size(); i++){
//					CarSalesProductCtrl c = ctrls.get(i);
//					if(c.getId() == ctrl.getId()&&!TextUtils.isEmpty(c.getUnLoadingCount())&&!c.getUnLoadingCount().equalsIgnoreCase("0")&&!c.getUnLoadingCount().equalsIgnoreCase("0.0")){
//						Toast.makeText(UnloadingActivity.this, "卸车列表已存在", Toast.LENGTH_LONG).show();
//						return;
//					}else{
//						int key = arr.keyAt(i);
//						EditText et = arr.get(key);
...
//				initProduct(product);
//				Intent i = new Intent(UnloadingActivity.this,UnLoadingListActivity.class);
//				startActivity(i);
			} else {
				ToastOrder.makeText(UnloadingActivity.this, "该产品不存在，请确认是否扫描错误",ToastOrder.LENGTH_LONG).show();
			}
		}
	}
	@Override
	...
	private void goUnLoadingList() {
		...
		List<CarSalesProductCtrl> productCtrlsChild = util.unLoadingProductCtrlList();
		if(productCtrlsChild!=null&&productCtrlsChild.size()>0){
			Intent intent = new Intent(UnloadingActivity.this,UnLoadingListActivity.class);
			Bundle bundle = new Bundle();
			bundle.putString("carSalesNo", SharedPreferencesForCarSalesUtil.getInstance(UnloadingActivity.this).getCarSalesNo());
			bundle.putInt("isRecord", 0);
			intent.putExtras(bundle);
			startActivityForResult(intent, 111);
		}else{
			ToastOrder.makeText(UnloadingActivity.this, "您没选择产品", ToastOrder.LENGTH_LONG).show();
		}
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		if (keyCode == KeyEvent.KEYCODE_BACK && event.getAction() == KeyEvent.ACTION_DOWN) {
			...
			Iterator<String> it = set.iterator();
			while(it.hasNext()){
				String s =ctrls.get(it.next()).getUnLoadingCount();
				if(!TextUtils.isEmpty(s)&&Double.parseDouble(s)!=0){
					count += 1;
				}
			}
			if (count > 0 ) {
				backDialog("退出后将清除卸车列表数据,是否确定退出?").show();
				return true;
			}else{
				finish();
			}
		...
		public void onReceive(Context context, Intent intent) {
			if (intent!=null && Constants.BROADCAST_CARSALES_CREATE_SUCCESS.equals(intent.getAction())) {
				try {
					...
					ToastOrder.makeText(UnloadingActivity.this, "常用列表数据更新异常", ToastOrder.LENGTH_SHORT).show();
				}
			}
		}
