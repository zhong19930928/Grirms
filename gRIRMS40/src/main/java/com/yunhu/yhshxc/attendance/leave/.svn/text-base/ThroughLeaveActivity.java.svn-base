 ...
 *
 */
public class ThroughLeaveActivity extends AbsBaseActivity implements OnItemClickListener{
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		tv_attend_leave.setText(PublicUtils.getResourceString(this,R.string.attendance_search));
		init();
						...
						try {
							...
							if ("0000".equals(resultCode)) {
								...
								if ("[]".endsWith(arrayStr)) {
									...
									if (pages == 1) {// 说明是刷新
										list.clear();
										newItems = new LeaveUtils(ThroughLeaveActivity.this).paseLeaveInfos(array,true);
									}else{
										newItems = new LeaveUtils(ThroughLeaveActivity.this).paseLeaveInfos(array,false);
									}
...
//		param.put("authDiv", "");
	...
	public void onItemClick(AdapterView<?> parent, View view, int position,
		...
		if(list.size()>0){
			...
			Intent intent = new Intent(ThroughLeaveActivity.this,PreviewLeaveActivity.class);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		switch (resultCode) { //resultCode为回传的标记，我在B中回传的是RESULT_OK
		   case 10:
			   pages = data.getIntExtra("pages", 1);
		    break;
		default:
		    break;
		    }
