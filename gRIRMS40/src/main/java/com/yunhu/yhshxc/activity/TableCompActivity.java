 ...
 *
 */
public class TableCompActivity extends AbsBaseActivity implements OnClickListener,ProductCodeListener,ReceiveLocationListener,GGMMCheckListener,SubmitDataListener{
	/**
	 * 表格中所有列的func的集合
	 */
	...
	private ProductCode productCode;//串码控件
	private ScanInputCode scanInputCode;//可输入扫描控件
	 ...
	 */
	private int compRow;
	private Map<String, ArrayList<Component>> rowMap;//表格中每一行控件 key是行号 value是该行所有控件的集合
...
//		}.start();
	...
	private void init(){
		...
		sv=(ScrollView)findViewById(R.id.sv_table);
		//初始化加载dialog
		initTableDialog = new MyProgressDialog(this,R.style.CustomProgressDialog,getResources().getString(R.string.initTable));
...
//		initTableDialog.setOnDismissListener(dialogDismissListener);
		...
		//保存模版按钮
		btn_stencil = (LinearLayout)findViewById(R.id.btn_stencil);
		btn_stencil.setOnClickListener(this);
		//表格内容
		ll_tableContent = (LinearLayout)findViewById(R.id.ll_tableContent);
		//表格标题
		ll_tableContentTitle = (LinearLayout)findViewById(R.id.ll_tableContentTitle);
		...
		//获取传过来的值
		if (isNormal) {
			bundle=getIntent().getBundleExtra("tableBundle");
			//获取级联关系
			chooseMapBundle=getIntent().getBundleExtra("chooseMapBundle");
			//获取组织结构关系
			orgMapBundle=getIntent().getBundleExtra("orgMapBundle");
			isNoWait = getIntent().getBooleanExtra("isNoWait", true);//是否无等待提交
		}
		//获取是否是超链接
...
//	};
	...
	private View addTableTitleRow() {
		...
		TextView tv_title;
		TextView delTv = new TextView(this);
		delTv.setTextSize(20);//设置文字大小
		delTv.setGravity(Gravity.CENTER);//设置居中
		delTv.setTextColor(Color.WHITE);//设置字体颜色
		delTv.setText(PublicUtils.getResourceString(this,R.string.delete));
//		delTv.setBackgroundColor(R.color.blue);
		...
		for (Func func : funcList) {
			...
			//如果有主键 就设置主键
			if(keyFunc == null && func.getIsImportKey() == 1 && (func.getType() == Func.TYPE_SINGLE_CHOICE_FUZZY_QUERY_COMP
																					|| func.getType() == Func.TYPE_SINGLE_CHOICE_FUZZY_OTHER_QUERY_COMP 
																					|| func.getType() == Func.TYPE_SELECTCOMP
...
//		return len;
	...
	public View addTableRow(String keyFuncStr) {
		...
		int i = 1;
		TableCell[] tableCell = new TableCell[tableLen+1];// 列数
		TableButtonComp delImg = new TableButtonComp(this);
		...
		for (final Func func : funcList) {
			if(func.getType() == Func.TYPE_HIDDEN){
				...
				if(keyFunc!=null && func.getIsEdit()==Func.IS_N){//如果有主键 并且设置不可用，则不能点击
				...
				scanInTableComp.getCurrentButton().setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						currentCompIndex = compList.indexOf(scanInTableComp);
						scanInputCode = new ScanInputCode(TableCompActivity.this,scanInTableComp.getFunc(),null);
						scanInputCode.setProductCodeListener(TableCompActivity.this);
				...
				productCodeInTableComp.getCurrentButton().setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						...
						productCode = new ProductCode(TableCompActivity.this,func,null);
						productCode.setProductCodeListener(TableCompActivity.this);
			...
			}else if(func.getType()==Func.TYPE_CAMERA || func.getType()==Func.TYPE_CAMERA_HEIGHT || func.getType()==Func.TYPE_CAMERA_MIDDLE || func.getType()==Func.TYPE_CAMERA_CUSTOM){//拍照
				...
				photoInTableComp.getCurrentButton().setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						...
						if (photoFlg == 2) {
							new PhotoPopupWindow(TableCompActivity.this, currentPhotoInTableComp, fileName).show(null);
						}else{
							currentPhotoInTableComp.startPhoto(fileName);
						}
					}
				});
				if(keyFunc!=null && func.getIsEdit()==Func.IS_N){
...
//	 * 添加表格中下拉框选择标识
//	 */
//	private void addTbleRowSelect(Func spinnerFunc,int rowNum,String value,String did){
//		String key = String.valueOf(spinnerFunc.getFuncId())+"#"+String.valueOf(rowNum);
//		String select = did+"#"+value;
//		SharedPreferencesUtilForTable.getInstance(this).setTableSelect(key, select);
//	}
	...
	private void removeTableRowSelect(Func spinnerFunc,int rowNum){
		String key = String.valueOf(spinnerFunc.getFuncId())+"#"+String.valueOf(rowNum);
		SharedPreferencesUtilForTable.getInstance(this).clear(key);
	}
	...
	private OnClickListener onTableRowClick = new OnClickListener() {
		...
		public void onClick(View v) {
			if (isAddDel) {
				...
				ToastOrder.makeText(TableCompActivity.this, R.string.save_sucessful2, ToastOrder.LENGTH_SHORT).show();
			}
		...
		confirm.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				for (int i = 0; i < subList.size(); i++) {//如果改行里面有拍照，删除sd卡下的照片
					...
					if((f.getType()==Func.TYPE_CAMERA || f.getType()==Func.TYPE_CAMERA_HEIGHT || f.getType()==Func.TYPE_CAMERA_MIDDLE) || f.getType()==Func.TYPE_CAMERA_CUSTOM && !TextUtils.isEmpty(value)){
						new FileHelper().deleteFile(Constants.SDCARD_PATH + value);
					}
					if (f.getType() == Func.TYPE_SELECTCOMP) {
						Dictionary dic = new DictDB(TableCompActivity.this).findDictListByTable(f.getDictTable(), f.getDictDataId(), value);
						if(dic !=null){
							int count = dic.getSelectCount();
							if (count>=1) {
								count = count -1;
								new DictDB(TableCompActivity.this).updateDictSelectCount(f.getDictTable(), value, count);
							}
						}
	...
	public View addTableRow(HashMap<String,String> rowData) {
		...
		for (final Func func : funcList) {
			...
			if(funcTable.getDefaultType()!=null && funcTable.getDefaultType()==Func.DEFAULT_TYPE_SQL && isNeedAddData){
...
//			JLog.d(TAG, "currentTableCompValue==>" + currentTableCompValue);
			...
			if(func.getType() == Func.TYPE_HIDDEN){
				...
				photoInTableComp.compRow = compRow;
				View view = photoInTableComp.getObject();
				if(!TextUtils.isEmpty(currentTableCompValue)&& currentTableCompValue.endsWith(".jpg") && !isUseStencil){//不是空并且不是模版的情况
...
//		ll_tableContent.addView(rowView);
	...
	private Component getSelectComp(Func func) {
		...
		SpinnerComp select = new SpinnerComp(this, func,chooseMapBundle.getString(func.getFuncId()+""),orgMapBundle);
	...
	private Component getOtherSelectComp(Func func) {
		...
		SingleChoiceSpinnerComp singleChoiceSpinnerComp = new SingleChoiceSpinnerComp(this, func,chooseMapBundle.getString(func.getFuncId()+""),orgMapBundle);
	...
	private Component getFuzzyMultiChoiceSpinner(Func func){
		FuzzyMultiChoiceSpinnerComp fuzzyMultiChoiceSpinnerComp = new FuzzyMultiChoiceSpinnerComp(this, func,chooseMapBundle.getString(func.getFuncId()+""),orgMapBundle);
	...
	private Component getMultiChoiceSpinner(Func func){
		MultiChoiceSpinnerComp multiChoiceSpinnerComp = new MultiChoiceSpinnerComp(this, func,chooseMapBundle.getString(func.getFuncId()+""),orgMapBundle);
	...
	private Component getFuzzySingleChoiceSpinner(Func func){
		FuzzySingleChoiceSpinnerComp fuzzySingleChoiceSpinnerComp = new FuzzySingleChoiceSpinnerComp(this, func,chooseMapBundle.getString(func.getFuncId()+""),orgMapBundle);
		...
		if(!TextUtils.isEmpty(select.value)){
			...
			if(parentSelect!=null){
				...
				if(!TextUtils.isEmpty(s)){
					if(isOther(s) || parentSelectFunc.getType()==Func.TYPE_MULTI_CHOICE_SPINNER_COMP || parentSelectFunc.getType()==Func.TYPE_MULTI_CHOICE_FUZZY_QUERY_COMP){//如果是数字的话
			...
			if(isOther(select.value) || func.getType()==Func.TYPE_MULTI_CHOICE_SPINNER_COMP || func.getType()==Func.TYPE_MULTI_CHOICE_FUZZY_QUERY_COMP){//因为多选的时候值是多个的时候就无法验证数字 且多选不包含其他类型
	 ...
	 * 
	 * @param
	 * @return一个表格样式的view tableRow表的每一行
	 */
...
//			addTableRow();
			if (isAddDel) {
				...
				msg.obj=addTableRow("");
				handler.sendMessage(msg);
				ToastOrder.makeText(TableCompActivity.this, R.string.add_row, ToastOrder.LENGTH_SHORT).show();
			}else{
				ToastOrder.makeText(TableCompActivity.this,R.string.un_pre_add, ToastOrder.LENGTH_SHORT).show();
			}
			break;
		case R.id.btn_save:
					...
					if (codeControl()) {
						try {
							if (compList.isEmpty()) {// 如果是空则删除存储的该条记录
								...
								SubmitItem item = submitItemDB.findSubmitItemByParamNameAndType(String.valueOf(funcTable.getFuncId()),funcTable.getType());
								if (item != null) {
...
//							this.finish();
	...
	private void showStencil(){
		View contentView = View.inflate(this, R.layout.table_stencil_prompt, null);
		...
		contentView.findViewById(R.id.table_stencil_save).setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				try {
					...
					ToastOrder.makeText(TableCompActivity.this,R.string.toast_one1, ToastOrder.LENGTH_SHORT).show();
					JLog.e(e);
				}
			}
		});
	...
	private void saveStencil() throws NumberFormatException, DataComputeException{
		...
		if (tableStencil == null) {
			...
			ToastOrder.makeText(this, R.string.save_sucessful, ToastOrder.LENGTH_SHORT).show();
		}else{
	...
	private void useStencil(){
		...
		TableStencil tableStencil = tableStencilDB.findTableStencilByTargetIdAndFuncId(targetId, funcId);
		if (tableStencil == null) {
			ToastOrder.makeText(this, R.string.save_sucessful1, ToastOrder.LENGTH_SHORT).show();
		}else{			
			//添加模版数据
			isUseStencil=true;
	...
	private void stencilDialog(final TableStencil tableStencil) {
		final Dialog deleteDialog = new Dialog(this, R.style.transparentDialog);
		View view = View.inflate(this, R.layout.table_stencil_dialog, null);
		...
		confirm.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				deleteDialog.dismiss();
				TableStencilDB tableStencilDB = new TableStencilDB(TableCompActivity.this);
	...
	private String getTableValue() throws NumberFormatException, DataComputeException{
		...
		HashMap<String,String> table = null; //存放一行的控件的值 key是控件的funcID 值是控件要保存的值
		for (int i = 0; i < compSize; i += tableLen) {
			table=new HashMap<String, String>();
			int j = 0;//每一行可见func的索引
			for (Func currentFunc : funcList) {
				if(currentFunc.getType()==Func.TYPE_SINGLE_CHOICE_FUZZY_OTHER_QUERY_COMP || currentFunc.getType()==Func.TYPE_SELECT_OTHER){
					if(compList.get(i + j).value==null){
						compList.get(i + j).value="";
					}
					if(!TextUtils.isEmpty(compList.get(i + j).value) && !isInteger(compList.get(i + j))){//如果是其他类型的下拉框 不是数字说明选了其他
						table.put(String.valueOf(currentFunc.getFuncId()), "-1");//先存-1
						table.put(currentFunc.getFuncId()+"_other", compList.get(i + j).value);//再存值  此时的key 是控件的ID加_other
					}else{//没有选其他的情况
					...
					table.put(String.valueOf(currentFunc.getFuncId()),new FuncTreeForTable(this,currentFunc,compList.subList(i, i+tableLen)).result);
					...
					String hiddenValue = "";
					if(currentFunc.getDefaultType()!=null && (currentFunc.getDefaultType() == Func.DEFAULT_TYPE_COMPUTER)){
						hiddenValue = new FuncTreeForTable(this,currentFunc,compList.subList(i, i+tableLen)).result;//表格隐藏类型自动计算
					}else{
	 ...
	 * @throws DataComputeException 
	 * @throws NumberFormatException 
	 * @throws Exception 保存的时候将要提交的值转化成json的时候会抛异常
	 */
	...
	private void save() throws NumberFormatException, DataComputeException,Exception {
		...
		int submitId = submitDB.selectSubmitIdNotCheckOut(planId,wayId, storeId, targetId,targetType, Submit.STATE_NO_SUBMIT);
		if (submitId != Constants.DEFAULTINT) {// submitId！=0表示没有该条记录这时候要查询看是插入数据还是修改数据
			...
			boolean isHas=false;
			if(isLink){//超链接查询临时表
				isHas = new SubmitItemTempDB(this).findIsHaveParamName(
			...
			if (isHas) {// isHas=true表示有该条记录，此时修改
				...
				if(isLink){
					new SubmitItemTempDB(this).insertSubmitItem(submitItem);// 没有记录，插入
				}else{
...
//			SubmitManager.getInstance(this).submitData();
			SubmitManager.getInstance(this).submitData(isNoWait, TableCompActivity.this);
			JLog.d(TAG, "表格历史数据提交");
			
		}else{
			this.finish();
		}
		if (!TextUtils.isEmpty(menuType2)&&menuType2.equals(Menu.TYPE_VISIT+"")) {
	...
	private String getTableImageList(List<HashMap<String, String>> tablelist, List<Func> funcTableList) {
		...
		for (int i = 0; i < tablelist.size(); i++) { // 循环行数
			...
			for (int j = 0; j < funcTableList.size(); j++) {
				...
				if(func.getType()==Func.TYPE_CAMERA || func.getType()==Func.TYPE_CAMERA_HEIGHT || func.getType()==Func.TYPE_CAMERA_MIDDLE || func.getType()==Func.TYPE_CAMERA_CUSTOM){//拍照类型
	...
	public String getHiddenValue(Func func) {
		if (func.getDefaultType() != null) {
			...
			case Func.DEFAULT_TYPE_USER:// 返回当前用户
				return SharedPreferencesUtil.getInstance(this).getUserName();//name修改成ID 代要求09_21号
	...
	/**
	 * 初始化数据
	 * 1：如果数据库中有数据就根据数据库中的数据初始化页面
	 * 2：如果是sql类型的就根据sql传过来的值赋值  isNetCanUse=false表示是sql网络不通的情况下跳转过来
	 * 3： keyFunc!=null说明是有主键，这时候要平铺，如果没有主键就直接添加一行，有主键的时候如果设置成不可编辑则不能点击操作
	 * 
	 */
	private Dialog locationLoadingDialog;
	private void initData() {
		if (isNeedLocation() && isNormal) {//如果是拍照异常返回话就不需要再次定位了
			locationLoadingDialog  = new MyProgressDialog(this,R.style.CustomProgressDialog,getResources().getString(R.string.waitForAMoment));
			locationLoadingDialog.show();
//			new LocationFactory(TableCompActivity.this).startNewLocation(TableCompActivity.this,false);//不需要地址
			new LocationFactoy(TableCompActivity.this, this).startLocationHH();
		}else{
			loadUI();
		}
	}
	...
	private void setDefaultTypeTable(){
		if (isNormal) {
			...
			if(isLink){//如果是超链接，查询临时表中的数据
				item = new SubmitItemTempDB(TableCompActivity.this).findSubmitItem(null,Submit.STATE_NO_SUBMIT, planId,
						wayId, storeId, targetId,targetType, String.valueOf(funcTable.getFuncId()));
			}else{//不是超链接查询提交表中的数据
				item = new SubmitItemDB(TableCompActivity.this).findSubmitItem(null,Submit.STATE_NO_SUBMIT, planId,
						wayId, storeId, targetId,targetType, String.valueOf(funcTable.getFuncId()));
			}
			if (item != null) {
				...
				if(keyFunc!=null){//如果有主键就平铺
					keyFuncDicList = new DictDB(TableCompActivity.this).findDictList(keyFunc.getDictTable(),keyFunc.getDictCols(),keyFunc.getDictDataId(),keyFunc.getDictOrderBy(),keyFunc.getDictIsAsc(),null,null,null);
	...
	private boolean isCompletion() {
		...
		for (int i = 0; i < compList.size(); i++) {
			...
			if ( func.getIsEmpty() != null && func.getIsEmpty() == 1 
					&& func.getType() != Func.TYPE_SELECTCOMP 
					&& func.getType() != Func.TYPE_SINGLE_CHOICE_FUZZY_OTHER_QUERY_COMP 
					&& func.getType()!=Func.TYPE_SELECT_OTHER 
				...
				if (comp.isEmpty()) { // 如果该项为空
					...
					ToastOrder.makeText(this, func.getName()+PublicUtils.getResourceString(this,R.string.un_search_data2), ToastOrder.LENGTH_SHORT).show();
					break;
				}
			}
		}
	...
	/**
	 * 根据验证类型检测输入的值是否正确
	 * @param currentComponent 表格中的控件
	 * @param func 表格中控件的func
	...
	public boolean checkType(Component currentComponent, Func func) {
		...
		if (!TextUtils.isEmpty(currentComponent.value)
			...
			case Func.CHECK_TYPE_FIXED_TELEPHONE:
				...
				if (!flag) {
					ToastOrder.makeText(this, name+PublicUtils.getResourceString(this,R.string.input_corect_photo), ToastOrder.LENGTH_LONG).show();
				}
				break;
			case Func.CHECK_TYPE_IDNUMBER:
				flag = currentComponent.isIDNumber();
				if (!flag) {
					ToastOrder.makeText(this, name+PublicUtils.getResourceString(this,R.string.input_corect_photo1), ToastOrder.LENGTH_LONG).show();
				}
				break;
			...
			case Func.CHECK_TYPE_MAIL:
				...
				if (!flag) {
					ToastOrder.makeText(this, name+PublicUtils.getResourceString(this,R.string.reset_email), ToastOrder.LENGTH_LONG).show();
				}
				break;
			case Func.CHECK_TYPE_ZIPCODE:
				flag = currentComponent.isZipCode();
				if (!flag) {
					ToastOrder.makeText(this, name+PublicUtils.getResourceString(this,R.string.input_corect_photo2), ToastOrder.LENGTH_LONG).show();
				}
				break;
			case Func.CHECK_TYPE_NUMERIC:
				...
				if (func.getDataType().intValue() == Func.DATATYPE_DECIMAL) {// 如果是小数
					...
					if (!flag) {
						ToastOrder.makeText(this, name+PublicUtils.getResourceString(this,R.string.input_corect_photo3), ToastOrder.LENGTH_LONG).show();
					}
				} else {
					flag = isInteger(currentComponent);
					if (!flag) {
						ToastOrder.makeText(this, name+PublicUtils.getResourceString(this,R.string.input_corect_photo4), ToastOrder.LENGTH_LONG).show();
					}
				}
				break;
			}
	...
	private boolean isPass(){
		...
		for (int i = 0; i < compList.size(); i++) {
			...
			if (func.getType() != Func.TYPE_SELECTCOMP 
					...
					&& func.getType()!=Func.TYPE_MULTI_CHOICE_FUZZY_QUERY_COMP 
					&& func.getType()!=Func.TYPE_MULTI_CHOICE_SPINNER_COMP 
					&& !TextUtils.isEmpty(comp.value)) {// 说明该项必须操作
		...
		for (int i = 0; i < compList.size(); i++) {
			...
			if (func.getType() == Func.TYPE_PRODUCT_CODE && !TextUtils.isEmpty(comp.value)) {
				...
				if ("3".equals(codeControl) || "4".equals(codeControl)) {
					...
					if (!isCanSubmit) {
						ToastOrder.makeText(TableCompActivity.this, R.string.toast_one, ToastOrder.LENGTH_SHORT).show();
						break;
					}
				}
			}
...
//				setScreenRotation(true);
	...
	private boolean isNeedLocation(){
		...
		for (int i = 0; i < funcList.size(); i++) {
			...
			if (func.getOrgOption()!=null && func.getOrgOption() == Func.ORG_STORE && func.getIsAreaSearch() == 1 && func.getAreaSearchValue() != null) {
				flag = true;
				break;
			}
		}
...
//			return PublicUtils.convertDIP2PX(this,func.getColWidth())+defaultValue;
			return PublicUtils.convertDIP2PX(this,Math.abs(func.getColWidth()))+getDateForDPI(densityDpi, 160);
		}else{//如果为空就默认返回下拉框的宽
			return defaultValue;
		}
	}
...
//        float density = metric.density;       // 屏幕密度（0.75 / 1.0 / 1.5）
	 ...
	 * @param currentDPI 当前手机的屏幕密度
	 * @param normal 显示正常的情况的值
	 * @return 适配后返回的值
	 */
	private int getHeighDateForDPI(int currentDPI,int normal){
		switch (currentDPI) {
		case 120:
			return PublicUtils.convertPX2DIP(this, (currentDPI*normal)/240)-40;
		case 160:
			return PublicUtils.convertPX2DIP(this, (currentDPI*normal)/240)-30;
		case 240:
			return PublicUtils.convertPX2DIP(this, (currentDPI*normal)/240);
		default:
			return PublicUtils.convertPX2DIP(this, (currentDPI*normal)/240)+(int)getResources().getDimension(R.dimen.table_col_heigh);
		}
	...
	private int getDelWidthDataForDPI(int currentDPI,int normal){
		...
		case 240:
			return PublicUtils.convertPX2DIP(this, (currentDPI*normal)/240);
		default:
			return PublicUtils.convertPX2DIP(this, (currentDPI*normal)/240)+(int)getResources().getDimension(R.dimen.table_del_width);
		}
	}
	...
	private int getDateForDPI(int currentDPI,int normal){
		...
		case 240:
			return PublicUtils.convertPX2DIP(this, (currentDPI*normal)/240);
		default:
			return PublicUtils.convertPX2DIP(this, (currentDPI*normal)/240)+(int)getResources().getDimension(R.dimen.table_col_width);
		}
	...
	/**
	 * 控制表格滑动到最底部
	 * @param scroll 滑动的scrollView 
	 * @param inner scrollView所在的父view
	 */
	...
	private String fileName;//拍照照片名字
	...
	private void resultForProductCode(String code){
		if (productCode == null) {
			...
			productCode = new ProductCode(TableCompActivity.this,productCodeInTableComp.getFunc(),null);
			productCode.setProductCodeListener(TableCompActivity.this);
			productCode.setProductTableType(ProductCode.TABLE_TYPE_TABLE);
			productCode.setTableRow(productCodeInTableComp.compRow);
			productCode.productCodeInTable(productCodeInTableComp, productCodeInTableComp.value);
...
//		}
		...
		if (currentPhotoInTableComp!=null) {
			...
			Func func = currentPhotoInTableComp.getFunc();
			...
			if (waterMark == 1) {
				...
				currentPhotoInTableComp.value=fileName;
				currentPhotoInTableComp.setName(PublicUtils.getResourceString(this,R.string.al_take_phpto));
			}
	...
	private void waterMarkDialog(final PhotoInTableComp currentPhotoInTableComp,final Date date){
		...
		confirmBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				currentPhotoInTableComp.resultPhoto(fileName,date);
				currentPhotoInTableComp.value=fileName;
				currentPhotoInTableComp.setName(PublicUtils.getResourceString(TableCompActivity.this,R.string.al_take_phpto));
				dialog.dismiss();
			}
		});
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (Activity.RESULT_OK == resultCode || resultCode == R.id.scan_succeeded) {
			...
			}else{//非正常返回，并且加载UI线程还没执行完毕 此时保存返回的值和code 等UI加载完毕执行unNormalActivityResult
...
//						Func func = comp.compFunc;
//						if (func !=null && func.getType() == Func.TYPE_SELECT_OTHER || func.getType() == Func.TYPE_SELECTCOMP
//								|| func.getType() == Func.TYPE_SINGLE_CHOICE_FUZZY_OTHER_QUERY_COMP || func.getType() == Func.TYPE_SINGLE_CHOICE_FUZZY_QUERY_COMP) {
//							
...
//						}
	...
	public void onReceiveResult(LocationResult result) {
		...
		if (result!=null && result.isStatus()) {
			...
			ToastOrder.makeText(this, getResources().getString(R.string.org_store_distance_location_fail), ToastOrder.LENGTH_SHORT).show();
		}
		loadUI();
	}
...
//		Settings.System.putInt(getContentResolver(),Settings.System.ACCELEROMETER_ROTATION,flag==1?0:1);
	...
	public void submitReceive(boolean isSuccess) {
		if (isSuccess) {
			ToastOrder.makeText(this, R.string.submit_ok, ToastOrder.LENGTH_LONG).show();
			this.finish();
		}
	}
