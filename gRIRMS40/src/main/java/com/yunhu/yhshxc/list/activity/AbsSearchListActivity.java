 ...
 * 
 */
public abstract class AbsSearchListActivity extends AbsBaseActivity {
	 ...
	 * 显示补报信息ListView
	 */
	public ExpandableListView elv_search;
	...
	private void fail(String message) {
		...
		ToastOrder.makeText(AbsSearchListActivity.this, message, ToastOrder.LENGTH_LONG).show();
	}
	...
	protected void afterSuccessParse() {
		if (dataList == null || dataList.isEmpty()) {
			...
			if (layout_id == R.layout.search_list) {
	...
	protected void init() {
		...
		if (layout_id == 0) {
			layout_id = R.layout.search_list;
		}
		// 设置activity显示布局
		setContentView(layout_id);
		// 获取ListView
		elv_search = (ExpandableListView) findViewById(R.id.elv_search_list);
		initBase();

		// 隐藏窗体
		if (layout_id == R.layout.search_list) {// 调用默认布局
		...
		if (layout_id == R.layout.search_list || modType == Constants.MODULE_TYPE_PAY) {// 默认布局
			// 对title进行初始化
			initTitleView();
		}
...
//		}
	...
	protected void netWorkInThread() {
		...
		// // 获取联网查询的数据
		// String result = new
		// HttpHelper(AbsSearchListActivity.this).connectPost(baseUrlSearch,
		// params);
		// // 新建一个Message
	...
	protected int computeViewWidth(int showCount, Func func) {
		...
		int realDIP = PublicUtils.convertPX2DIP(AbsSearchListActivity.this,
				realPX);
		...
		return PublicUtils.convertDIP2PX(AbsSearchListActivity.this,
		...
		// int viewWidth = 100;
		// int tempWidth = realDIP/showCount;
		// return PublicUtils.convertDIP2PX(AbsSearchListActivity.this,viewWidth
		// >= tempWidth ? viewWidth : tempWidth);
	}

	/**
	 * ExpandableListView的Adapter
	 * getChildrenCount()由子类getCurChildrenCount()方法实现；
	 * getGroupView()由子类getCurGroupView()方法实现；
	 * getChildView()由子类getCurChildView()方法实现；
	 * 
	 * @see BaseExpandableListAdapter
	 */
	private final class SearchAdapter extends BaseExpandableListAdapter {
		...
		public int getChildrenCount(int groupPosition) {
			return getCurChildrenCount(groupPosition);
		}
		...
		public View getGroupView(int groupPosition, boolean isExpanded,
			...
			return getCurGroupView(groupPosition, isExpanded, convertView,
					parent);
		}
		...
		public View getChildView(int groupPosition, int childPosition,
			...
			return getCurChildView(groupPosition, childPosition, isLastChild,
					convertView, parent);
		}
		...
		if (!TextUtils.isEmpty(mIds)) {// 判断非空
			if (func.getOrgOption() != null
				...
				List<OrgStore> orgStoreList = new OrgStoreDB(
						AbsSearchListActivity.this).findOrgListByStoreId(mIds);
				...
				List<OrgUser> orgUserList = new OrgUserDB(
						AbsSearchListActivity.this).findOrgUserByUserId(mIds);
				StringBuffer sb = new StringBuffer();
				...
				List<Org> orgList = new OrgDB(AbsSearchListActivity.this)
						.findOrgByOrgId(mIds);
				StringBuffer sb = new StringBuffer();
				...
				if(!",".equals(mIds)){
					tvValue = new DictDB(AbsSearchListActivity.this)
	...
	protected String getSelectCompValue(Func func, String key) {
		...
		if (!TextUtils.isEmpty(key)) {// 判断非空
			if (func.getOrgOption() != null
				...
				List<OrgStore> orgStoreList = new OrgStoreDB(
						AbsSearchListActivity.this).findOrgListByStoreId(key);
				StringBuffer sb = new StringBuffer();
				for (int j = 0; j < orgStoreList.size(); j++) {
					OrgStore orgStore = orgStoreList.get(j);
					sb.append(",").append(orgStore.getStoreName());
				...
				List<OrgUser> orgUserList = new OrgUserDB(
						AbsSearchListActivity.this).findOrgUserByUserId(key);
				StringBuffer sb = new StringBuffer();
				for (int j = 0; j < orgUserList.size(); j++) {
					OrgUser orgUser = orgUserList.get(j);
					sb.append(",").append(orgUser.getUserName());
				...
				// 获取下拉内容Dictionary
				Dictionary dic = new DictDB(AbsSearchListActivity.this)
