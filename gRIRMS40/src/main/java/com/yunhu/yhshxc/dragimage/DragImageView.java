	...
	void setScale(float scale) {
		...
		if (scale > 1 && this.getWidth() <= MAX_W) {
			...
			if (current_Top <= 0 && current_Bottom >= screen_H) {
				Log.e("jishen", "屏幕高度=" + this.getHeight());
	...
	class MyAsyncTask extends AsyncTask<Void, Integer, Void> {
		...
		protected Void doInBackground(Void... params) {
			...
			while (current_Width <= screen_W) {
                ...
                Log.e("jishen", "top="+top+",bottom="+bottom+",left="+left+",right="+right);
                isContinue(new Integer[] { left, top, right, bottom });
				try {
					Thread.sleep(10);
		...
		private void isContinue(final Integer... values){
			//4.1版本以上
		...
		private Handler mHandler = new  Handler(){
			public void handleMessage(Message msg) {
				final Integer[] values =  (Integer[]) msg.obj;
				mActivity.runOnUiThread(new Runnable() {
					@Override
					public void run() {
						setFrame(values[0], values[1], values[2], values[3]);
