...
import com.yunhu.yhshxc.parser.AttendanceParse;
...
public class AttendanceListActivity extends AbsSearchListActivity {
	...
	protected List getDataListOnThread(String json) throws Exception {
		// 解析json,获取考勤信息列表
		List<Attendance> attendanceList = new AttendanceParse()
	...
	protected int getCurChildrenCount(int groupPosition) {
		Attendance attendance = (Attendance) dataList.get(groupPosition);
	...
	protected View getCurGroupView(int groupPosition, boolean isExpanded,
		...
		if (convertView == null) {// 没有缓存对象
			// 初始化view
			convertView = View.inflate(AttendanceListActivity.this,
		...
		// 设置考勤日期
		groupHolder.tv_date.setText(attendance.getCreateTime());
		...
		if (isExpanded) {// 已展开
			...
			groupHolder.ll_color
					.setBackgroundResource(R.color.att_bg_color_blue);
		} else {// 未展开
			...
			if (!TextUtils.isEmpty(attendance.getInTime())) {// 上班
				if (!TextUtils.isEmpty(attendance.getOutTime())) {// 下班
					groupHolder.ll_color
							.setBackgroundResource(R.color.att_bg_color_normal);
				} else {
					groupHolder.ll_color
							.setBackgroundResource(R.color.att_bg_color_yellow);
				}
			} else {
				groupHolder.ll_color
						.setBackgroundResource(R.color.att_bg_color_red);
			}
		}
		return convertView;
	}
	...
	protected View getCurChildView(int groupPosition, int childPosition,
		...
		if (childPosition == 0) {// 上班
			att_state = PublicUtils.getResourceString(AttendanceListActivity.this,R.string.s_work)+"  :  ";
		...
		} else if (childPosition == 1) {// 下班
			att_state = PublicUtils.getResourceString(AttendanceListActivity.this,R.string.e_work)+"  :  ";
		...
		final String imageName = attendance.getCreateTime() + " - "
	...
	private String getTypeValue(String att_type) {
		...
		// value = "GPS卫星定位";
		// }
		// else if ("2".equals(att_type)) {
		// value = "GPSOne混合定位";
		// }
		// else if ("3".equals(att_type)) {
		// value = "优化基站定位";
		// }
		if ("1".equals(att_type)) {
			value = PublicUtils.getResourceString(AttendanceListActivity.this,R.string.loation_type);
		} else {
			value = PublicUtils.getResourceString(AttendanceListActivity.this,R.string.loation_type1);
		}
		return value;
	}
	...
	protected void setPageParams(HashMap<String, String> params) {
		if (dataList != null && !dataList.isEmpty()) {
			Attendance attendance = (Attendance) dataList
					.get(dataList.size() - 1);
			params.put(PAGE, attendance.getInTime());
	...
	public void addPaging() {
		...
		loadMoreButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				loadMoreButton.setText(PublicUtils.getResourceString(AttendanceListActivity.this,R.string.init));// 设置按钮文字
	...
	public void dismissLoadDialog() {
		...
		if (loadMoreButton != null) {
			...
			loadMoreButton.setText(PublicUtils.getResourceString(AttendanceListActivity.this,R.string.check_more));
		}
	}
