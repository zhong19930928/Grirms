 ...
 *
 */
public class ZCLRFuncActivity extends AbsFuncActivity implements GGMMCheckListener, SubmitDataListener {
...
//	private List<Map<String, String>> dataList ;
	...
	private void init() {
		...
		dataList = (List<Map<String, String>>) bundle.getSerializable("list");
	...
	public void initLayout() {
		...
		if (modType == Constants.MODULE_TYPE_ISSUED) {// 改派和下发两种类型的显示用户
			...
			userFunc.setName(PublicUtils.getResourceString(ZCLRFuncActivity.this,R.string.func_name));
...
//								previewDataComp.setShowDataTitle(name);
								...
								if (currentValue.split("\\$").length > 4) {
									buffer.append("\n").append(PublicUtils.getResourceString(ZCLRFuncActivity.this,R.string.func_name1)).append(currentValue.split("\\$")[4]);
								}
...
//				}
			...
			if (linkId != Constants.DEFAULTINT) {// 超链接预览
				...
				ToastOrder.makeText(ZCLRFuncActivity.this, getResources().getString(R.string.priview),
...
//				thisFinish(R.id.submit_no_succeeded, null);
	...
	private boolean codeSubmitControl() {
		...
		if (codeFunc != null) {
			...
			if (!flag) {
				ToastOrder.makeText(this, PublicUtils.getResourceString(ZCLRFuncActivity.this,R.string.toast_one), ToastOrder.LENGTH_SHORT).show();
					...
					if (item == null || TextUtils.isEmpty(item.getParamValue())) {
						if (!TextUtils.isEmpty(isEmptyFunc.getEnterMustList())) { // 如果此处有值，要判断条件是否满足，满足时，才是必填
							...
							try {
								...
								ToastOrder.makeText(ZCLRFuncActivity.this, e.getMessage(), ToastOrder.LENGTH_SHORT)
										.show();
								JLog.e(e);
								break;
							}
							if (isEmptyByEnterMustList) { // 如果不满足，为不必填
								isCompletion = false;
								ToastOrder.makeText(ZCLRFuncActivity.this,
										PublicUtils.getResourceString(ZCLRFuncActivity.this,R.string.toast_one2)+"\n\"" + isEmptyFunc.getName() + "\"", ToastOrder.LENGTH_SHORT).show();
								break;
							} else {
								continue;
							}
							...
							isCompletion = false;
							ToastOrder.makeText(ZCLRFuncActivity.this, PublicUtils.getResourceString(ZCLRFuncActivity.this,R.string.toast_one2)+"\"" + isEmptyFunc.getName() + "\"",
	...
	public void submitReceive(boolean isSuccess) {
		...
		if (isSuccess) {
			...
			Toast.makeText(ZCLRFuncActivity.this, R.string.submit_ok, Toast.LENGTH_SHORT).show();
			...
			if (submit != null) {
				submit.setState(Submit.STATE_NO_SUBMIT);
				submitDB.updateSubmit(submit);
				Toast.makeText(ZCLRFuncActivity.this, R.string.toast_one7, Toast.LENGTH_SHORT).show();
				;
			}
		}
	}
