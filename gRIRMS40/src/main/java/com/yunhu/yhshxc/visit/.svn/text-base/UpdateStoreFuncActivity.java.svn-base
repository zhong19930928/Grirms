...
/**
 * 店面信息修改的页面
 * 修改店面信息，根据后台传过来的原始数据进行修改，控件的初始值为后台传过来的原始数据
 * @author jishen
 *
 */
public class UpdateStoreFuncActivity extends AbsFuncActivity implements SubmitDataListener{
	...
	public String titleName(){
		return PublicUtils.getResourceString(this,R.string.modify_store_info);
	}
	...
	private void init(){
		...
		status=bundle.getString("status");//二级菜单类型所获取的控件
		infoType = bundle.getInt("infoType",0);//店面修改类型
		menuType=Menu.TYPE_MODULE;//设置为自定义类型
	...
	public List<Func> getFuncList() {
		...
		List<Func> funcList = funcDB.findFuncListByTargetid(targetId,true);//查询可修改的控件
		return funcList;
	}
...
//		linkBundle.putInt("infoType", infoType);//店面信息修改
...
//		intent.putExtra("ableStatus", status);//二级菜单控件状态
//		intent.putExtra("modType", modType);//自定义模块类型
//		intent.putExtra("menuType", menuType);//菜单类型
//		intent.putExtra("infoType", infoType);//店面信息标识
		try {
...
//		time = "12:00|0.5,14:55|0.5";
		...
		if (menuType == Menu.TYPE_ORDER3) {
			if (flag) {//时间过了判断天
				...
				if (!flag) {
					showTip(PublicUtils.getResourceString(UpdateStoreFuncActivity.this,R.string.toast_one8));
					submitDataLayout.setEnabled(true);
				}
			}else{
					...
					if (item == null || TextUtils.isEmpty(item.getParamValue())) {
						if (!TextUtils.isEmpty(isEmptyFunc.getEnterMustList())) { // 如果此处有值，要判断条件是否满足，满足时，才是必填
							...
							try {
								...
								ToastOrder.makeText(UpdateStoreFuncActivity.this,e.getMessage(),ToastOrder.LENGTH_SHORT).show();
								JLog.e(e);
								break;
							}
							if (isEmptyByEnterMustList) { // 如果不满足，为不必填
								isCompletion = false;
								ToastOrder.makeText(UpdateStoreFuncActivity.this,PublicUtils.getResourceString(UpdateStoreFuncActivity.this,R.string.toast_one2)+"\n\"" + isEmptyFunc.getName() + "\"",ToastOrder.LENGTH_SHORT).show();
								break;
							} else {
								continue;
							}
							...
							isCompletion = false;
							ToastOrder.makeText(UpdateStoreFuncActivity.this,PublicUtils.getResourceString(UpdateStoreFuncActivity.this,R.string.toast_one2)+"\"" + isEmptyFunc.getName() + "\"",ToastOrder.LENGTH_SHORT).show();
							break;
						}
					}
				}
...
//			thisFinish(R.id.submit_succeeded,intent);
			...
			Toast.makeText(UpdateStoreFuncActivity.this, R.string.submit_ok, Toast.LENGTH_SHORT).show();;
		}else{
			if (submit != null) {
				submit.setState(Submit.STATE_NO_SUBMIT);
				submitDB.updateSubmit(submit);
				Toast.makeText(UpdateStoreFuncActivity.this, R.string.toast_one7, Toast.LENGTH_SHORT).show();;
			}
		}
	}
